...
const CATALOGUE_PRICE_MAP = {
  ...
  TAND007: { name: "Tandoori Platter", price: 500 }, TAND008: { name: "Fish Tikka", price: 200 },
...
async function sendMainMenu(to, page = 0) {
  ...
  await sendList(to, "🍽️ Menu Categories", "Authentic Chettinad flavours! 🌶️\nSelect a category:", "Browse Menu", [{ title: "🍽️ Categories", rows }]);
}
...
async function placeOrder(from, session) {
  ...
  const { name, phone, address, order_type, paymentMethod, addons,
    ...
    table_persons, table_date, table_time, table_seating, pickup_time } = session.deliveryData;
...
async function sendPaymentButtons(from, order_type) {
...
async function generateRazorpayLink(session, from, type = "upi") {
  ...
  const expiry = type === "upi" ? 1800 : 3600; // 30 min UPI, 60 min card
  try {
...
const handleMessage = async (from, messageBody, interactiveReply, locationData, catalogueOrder) => {
  try {
    ...
    // ── VIEW CATALOGUE — direct, no extra image ──────────
    ...
    if (session.state === "SEARCHING" && rawInput && !rawInput.startsWith("/") && input !== "__FLOW_COMPLETE__") {
      ...
      await sendList(from, "🔍 Search Results", `Found ${results.length} item(s) for "${rawInput}":`, "View Items", [{ title: "Results", rows }]);
    ...
    // ── SHARE LOCATION → prompt pin drop ─────────────────
    if (input === "SHARE_LOCATION") {
      session.state = "AWAITING_LOCATION";
      await session.save();
      await sendText(from, "📍 *Share your location:*\n\nTap the 📎 attachment icon below → *Location* → *Send your current location*\n\nWe will use it as your delivery address!");
      await sendButtons(from, "Or tap below if you'd like to choose a different location method:", [
        { id: "TYPE_ADDRESS", title: "✏️ Type Address Instead" },
      ]);
      return;
    }

    // ── TYPE ADDRESS → open flow ──────────────────────────
    ...
    if (input === "__FLOW_COMPLETE__") {
      ...
      // Use live location if shared before flow
      ...
      const isDelivery     = order_type === "delivery";
      const deliveryCharge = isDelivery ? (within_five_km === "yes" ? 100 : 150) : 0;
